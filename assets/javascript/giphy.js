...
// list of dinosaur buttons
var topics = [
    'iguanodon', 'triceratops', 'velociraptor', 'ankylosaurus', 'archaeopteryx',
    'brachiosaurus', 'spinosaurus', 'tylosaurus', 'pachycephalosaurus', 'parasaurolophus', '(╯°□°)╯→'
];
...
function addButtons() {
    // empty out the buttons to prevent appending to the old ones
    $('#buttonArea').empty();
    // for all the topics in the topics array, add a button
    for (var i = 0; i < topics.length; i++) {
        // append them to the button area
        $('#buttonArea').append(
            // give it the primary class and dino-button class for style and
            // use later
            $('<button>').addClass('btn btn-primary dino-button')
...
function addDinoAdderForm() {
    // create form
    var dinoAdder = $('<form>');
    // add the label for the button addition input
    dinoAdder.append($('<label>').attr('for', 'addDino').text('🔍 Add Dinosaur:'))
        ...
        .append($('<button>').addClass('btn btn-info add-dino').text('Add'));
    // add the form to the container
    $('.container').append(dinoAdder);
}

function addDinoButton() {
    // when the add-dino button is clicked
    $('.add-dino').on('click', function() {
        // push the value to the topics array
        topics.push($('#addDino').val().trim());
...
function addGifs() {
    // on click of a dino-button
    $(document).on('click', '.dino-button', function() {
        ...
        var queryURL = 'https://api.giphy.com/v1/gifs/search?q=' +
            searchTerm + '&limit=10&api_key=T2CfMdH06QQJKeM3OEsBujVu9awpw7yb';
